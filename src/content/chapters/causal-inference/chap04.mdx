...
<Pitfall title="结局模型误设无法对冲">

G 计算对结局模型的依赖是单向的、不可对冲的。如果结局模型错了，没有第二个模型可以补救。在实践中检测模型误设的方法包括：检查残差分布、比较不同函数形式下的 G 计算结果、在关键协变量上加非线性项和交互项后观察 RD 是否发生实质变化。如果加入 APACHE 评分的二次项后 RD 从 0.060 变成 0.070，说明线性假设对估计有影响，应该考虑更灵活的模型。
