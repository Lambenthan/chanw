...
理解 Boosting 与 Bagging 的差异——序列纠错对照并行平均，把 eta、max_depth、subsample 与 L1/L2 正则放进同一张参数表并用早停作为最重要的正则化手段，在 0.66% 不平衡的 Bao 数据上标定 scale_pos_weight，用 Bao 时间切分上的 1991–2008 训+验、2009–2014 测，看清验证集 AUC 与测试集 AUC 之间的时间外推塌陷。
