...
随机森林还白送一个"OOB 估计"。每次 bootstrap 抽样大约 $1 - 1/e \approx 36.8\%$ 的样本没被抽到，被这棵树视为 OOB 样本。把没看见样本的那些树拉出来给该样本打分再平均，就是 OOB 预测。这个预测在原理上等价于一次留出验证。但在不平衡数据下，"OOB 看起来很好"几乎不携带判别信息，本章后面的雷区会展开。
